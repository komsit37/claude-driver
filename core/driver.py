...
class BrowserController:
    ...
    def _analyze_page_structure(self, html_content, url, title):
        ...
        analysis = {
            ...
            "detected_patterns": {
                "potential_items": len(soup.find_all(attrs={"data-claude-container": "potential-item"})),
                "prices_detected": len(soup.find_all(attrs={"data-claude-price": "detected"})),
                "dates_detected": len(soup.find_all(attrs={"data-claude-date": "detected"}))
            },
    ...
    def _suggest_selectors(self, soup):
        ...
        for elem in soup.find_all('div', class_=True):
            ...
            for cls in classes:
                if any(keyword in cls.lower() for keyword in ['item', 'order', 'product', 'card', 'box']):
